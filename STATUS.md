...
**`data/predictions/RESULTS.md` is the authoritative results file.** It is
generated by `pipeline/evaluate.py` from the same data as `metrics.json`, so it
cannot drift. Read it rather than trusting a table copied into prose, including
the summary below, which is correct as of this commit only.

It marks which conditions cover all 28 cases and which do not. **Never tabulate
an incomplete condition against a complete one.** That mistake has already been
made once in this file: an earlier version reported C0 as having the best F1,
which was true only while C2 covered 23 of 28 cases. With all 28, C2 overtakes
it.

### Two metric corrections that must survive into the report

**Never report `hit@k` alone.** A model that calls half a transaction a TRIGGER
scores well on hit@any by volume. Precision and the trigger rate must sit beside
it.

**Never compare precision across transactions of different size without lift.**
The share of calls the benchmark marks as vulnerable falls with size: 23.1% in
transactions of ten calls or fewer, 2.8% in those of 41 to 100. Precision
therefore falls automatically. `lift` divides that out (precision over the rate
random marking achieves; 1.0 is chance). An earlier version of this file claimed
performance collapses on large traces; once lift is used the effect vanishes and
the claim is withdrawn.

### Headline, as of this commit

| Condition | hit@1 | Precision | Recall | F1 | Called TRIGGER | Benchmark | Lift |
|-----------|-------|-----------|--------|-----|----------------|-----------|------|
...
C3 to C5 were still running when this was written. Check `RESULTS.md`.

### What to write in Chapter 6

Three claims, all measured:

1. **Names are what make the model selective, and they are worth more than the
   rule baseline.** With every identifier masked (C1) the model labels 40.7% of
   calls TRIGGER and scores below the rules (F1 0.245 against 0.322). Given
   decoded names (C2) it labels 25.6% and overtakes them (F1 0.345, lift 3.15).
   One variable, one attributable effect: this is the ablation working.
2. **Every condition still over-labels TRIGGER by three to six times.** The
   benchmark marks 7.0% of calls; the best condition marks 25.6%. Recall is high
   (0.70-0.84) and precision never exceeds 0.22. The model locates the flawed
   call but cannot separate it from the setup that enabled it or the drain it
   caused. Even the best condition sits near 3x chance.
3. **The margin over a transparent rule baseline is small.** C2 beats C0 by
   0.02 F1. An LLM costing 148,471 tokens barely outperforms a lexical rule
   costing nothing. Report that honestly; it is the most useful sentence in the
   chapter for anyone deciding whether to deploy this.

That is a mixed result with a clear mechanism, which is worth more than a single
accuracy figure. The study also caught two of its own metric errors, and both
